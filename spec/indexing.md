...
participating engines that something moved rather than waiting for them to notice.
...
**The record is keyed by path and holds `lastmod`, not merely membership.** Remembering only
which URLs have been seen would mean an edited article is never announced again -- the one event
the protocol most wants -- while remembering the modification time makes "new" and "changed" the
same comparison. The sitemap already carries `lastmod`, so this costs nothing to know.
