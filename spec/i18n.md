...
## The default runner is the open-weight one

A command given no `--model` uses `gpt-oss`. Nothing here needs the strongest model available
-- describing a picture and naming what is in it are reading tasks -- and a default that
spends the most expensive allowance is one that gets overridden every time it is used.
