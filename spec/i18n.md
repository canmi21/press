...
translation or summary sidecar may not exist when its virtual article module first loads. Any
such create or atomic replacement explicitly invalidates that virtual module; generated prose
must become visible without restarting the dev server.

In development, a Markdown change refreshes the segment layout with the Rust implementation
before it invalidates the virtual article module. Saves arriving in one burst are coalesced, and
the resulting layout-file event is consumed by the same update rather than rebuilding the whole
article table twice. This removes the stale-layout window without duplicating segmentation in
TypeScript; CI and production builds continue to consume the committed record without Rust.

The virtual module stores each distinct compiled locale payload once and reconstructs the locale
lookup with shared references. A missing translation points back to the source payload rather
than serializing the same blocks again for every locale. Source fallback is semantically one view;
letting JSON expand it into nine copies makes each development rebuild retain an article-sized
module generation and turns ordinary editing into heap growth.
