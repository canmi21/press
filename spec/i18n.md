...
with a separate tool, and are not covered here yet.
...
scalar untouched. Missing body translations still fail the build; missing titles, subtitles,
and descriptions deliberately fall back to their source values so metadata never renders as
empty copy while a translation run is incomplete.
...
does not. `cms i18n` and `cms summary` both skip anything without one, and the site compiles
every view of a page from the source.
