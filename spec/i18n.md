...
## Same-language views preserve the article

`zh-CN` and `zh-TW` are target views even when the article itself is Chinese, but they are not
an invitation to rewrite it. The exact source locale is materialised locally from each segment
with `source` provenance and zero tokens; sending prose the author already supplied to a model
can only spend money paraphrasing it. The stored copy is considered stale if its normalised
character-bigram similarity to the source falls below 0.9, which also repairs older generated
sidecars instead of trusting their presence.

A same-language sibling target still needs script or orthography conversion, but nothing else:
claims, sentence boundaries, word choice, jokes, emphasis, deliberate minority-language
passages, and voice stay where the author put them. The same 0.9 boundary applies to a model
reply when the requested locale exactly matches the source. This is an acceptance boundary, not
merely prompt advice; it keeps the source-language view from becoming a paraphrase and from
being indexed as a competing version of the article.

Translator's notes are unnecessary in same-language views because their readers can already
read the wording whose effect would otherwise be lost. A recorded note therefore invalidates
only different-language translations.
