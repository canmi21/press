...
## A short form may not borrow a dash the author never spent

No em dash, en dash or fullwidth dash in a short title or a short subtitle, unless the field it
shortens already carries one. A hyphen is untouched: it builds words rather than splicing
sentences, and German and French need it to.
...
The check reads the field's own source: a short subtitle answers to the subtitle it shortens, not
...
The estimate charges Han and kana a full square and Hangul 13.84 of one, because those two are
near enough constant. Latin is a table of 137 measured advances instead, because an average there
is not a small error: `i` advances 4.03px and `W` 16.40, and a single figure chosen safely above
both charges an ordinary sentence about a tenth more than it draws.
