...
`data/article-segments.json`; `cms i18n` refreshes the same record before making any paid
...
id plus the start and end byte offset of each translatable source block. It has order because
the build needs order; the hand-edited sidecar remains a map and continues to know nothing
about it. It does not contain assembled views or duplicate article prose.
